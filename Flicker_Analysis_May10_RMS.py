...
def getRMS(signal):
    ### Function to calculate RMS amplitude from epochs. Signal is an MNE epoch object with data shape of (ntrials,nchannels,nsamples)
    #This is woefully hardcoded, but will work. Update to be more general.
    channelList = ['E2','H4','C1','H2','F1','D1','H3','H1']
    rms = {key: None for key in channelList}     #Create a dictionary to hold all data
    for channel in channelList:
        curChannel = signal[channel].get_data()
        [nx,ny,nz] = np.shape(curChannel)
        rmsVec = np.zeros((nx,ny))
        for x in range(nx):
            for y in range(ny):   
